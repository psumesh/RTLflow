...
struct RfUnpacked final {
    ...
    __device__ const T_Value& operator[](size_t index) const {
        return m_array[Num_Testbenches * index];
    }
};

template <std::size_t T_Words, std::size_t Num_Testbenches> struct RfWide final {
    EData* m_storage;

    __device__ const EData& operator[](size_t index) const {
        return m_storage[Num_Testbenches * index];
    };
