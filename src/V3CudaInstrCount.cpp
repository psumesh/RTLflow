...
class CudaInstrCountVisitor final : public AstNVisitor {
private:
    // NODE STATE
    ...
    private:
        ...
        AstNode* m_nodep;
        CudaInstrCountVisitor* m_visitor;

    public:
        // CONSTRUCTORS
        VisitBase(CudaInstrCountVisitor* visitor, AstNode* nodep, const std::string& additional = "")
            : m_nodep{nodep}
            , m_visitor{visitor} {
            m_savedCount = m_visitor->startVisitBase(nodep, additional);
        }
...
public:
    // CONSTRUCTORS
    CudaInstrCountVisitor(AstNode* nodep, bool assertNoDups, std::ostream* osp)
        : m_startNodep{nodep}
    ...
    virtual ~CudaInstrCountVisitor() override = default;

    // METHODS
    uint32_t instrCount() const { return m_instrCount; }

private:
    uint32_t startVisitBase(AstNode* nodep, const std::string& additional) {
        ...
        std::string key = nodep->typeName();
        uint32_t savedCount = m_instrCount;
        //printf("%s\n", (key + additional).c_str());
        m_instrCount = m_wt[key + additional] * nodep->cudaInstrCount();
        return savedCount;
    }
    ...
    virtual void visit(AstVarRef* nodep) override {
        ...
        if(nodep->varp()->isCuda()) {
          additional += "GLOBAL";
        }
        VisitBase vb(this, nodep, additional);
        iterateChildren(nodep);
    }

    virtual void visit(AstNode* nodep) override {
        //VisitBase vb(this, nodep, nodep->dtype() ? nodep->dtype()->isWide() : false);
        VisitBase vb(this, nodep);
        iterateChildren(nodep);
    }

    VL_DEBUG_FUNC;  // Declare debug()
    VL_UNCOPYABLE(CudaInstrCountVisitor);
};

// Iterate the graph printing the critical path marked by previous visitation
class CudaInstrCountDumpVisitor final : public AstNVisitor {
private:
    // NODE STATE
...
public:
    // CONSTRUCTORS
    CudaInstrCountDumpVisitor(AstNode* nodep, std::ostream* osp)
        : m_osp{osp} {
        // No check for nullptr output, so...
        UASSERT_OBJ(osp, nodep, "Don't call if not dumping");
        if (nodep) iterate(nodep);
    }
    virtual ~CudaInstrCountDumpVisitor() override = default;
...
private:
    ...
    VL_DEBUG_FUNC;  // Declare debug()
    VL_UNCOPYABLE(CudaInstrCountDumpVisitor);
};

uint32_t V3CudaInstrCount::count(AstNode* nodep, bool assertNoDups, std::ostream* osp) {
    CudaInstrCountVisitor visitor(nodep, assertNoDups, osp);
    if (osp) CudaInstrCountDumpVisitor dumper(nodep, osp);
